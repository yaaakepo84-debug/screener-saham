...
st.info(f"Universe: **{len(tickers)} kode saham IDX**. Daftar emiten tidak lagi bergantung pada Yahoo Screener; data harga tetap dari Yahoo Finance.")
...
if mode == "🌙 Watchlist Besok":
    ...
    if filtered.empty:
        ...
        table = filtered[["Kode", "Tier", "Kategori", "Entry Low", "Entry High", "TP1", "TP2", "SL", "RR_TP1", "RR_TP2", "Score", "RSI", "Vol x"]].copy()
...
# ------------------------------------------------------------
    ...
    st.subheader("🚀 BPJS — Beli Pagi Jual Sore")
    st.caption("Daily = filter kandidat • 15M = konfirmasi arah. Entry tetap perlu cek chart/bid-offer.")

    with st.sidebar:
        st.header("Filter BPJS")
        min_score_bpjs = st.slider("Minimal Score BPJS", 40, 100, 60, 5)
        max_candidates = st.slider("Kandidat awal", 10, 40, 20, 5)
        only_ready = st.checkbox("Tampilkan READY saja", value=False)

    daily_bpjs = scan_bpjs_daily(tickers)

    if daily_bpjs.empty:
        st.warning("Belum ada kandidat BPJS.")
        st.stop()

    daily_bpjs = daily_bpjs[daily_bpjs["Score"] >= min_score_bpjs].copy()
    daily_bpjs = daily_bpjs.sort_values(["Score", "Vol Daily x"], ascending=[False, False]).head(max_candidates)

    if daily_bpjs.empty:
        st.warning("Tidak ada kandidat setelah filter score.")
        st.stop()

    conf = scan_bpjs_intraday(daily_bpjs["Kode"].tolist())

    if not conf.empty:
        bpjs = daily_bpjs.merge(conf, on="Kode", how="left")
    else:
        bpjs = daily_bpjs.copy()
        bpjs["15M Confirm"] = "NO DATA"

    bpjs["15M Confirm"] = bpjs["15M Confirm"].fillna("WAIT")
    if only_ready:
        bpjs = bpjs[bpjs["15M Confirm"] == "READY"].copy()

    bpjs = bpjs.sort_values(["15M Confirm", "Score"], ascending=[True, False])

    table = bpjs[["Kode", "15M Confirm", "Score", "Close", "Entry Low", "Entry High", "TP1", "TP2", "SL", "RR_TP1", "RR_TP2", "RSI Daily", "Vol Daily x"]].copy()
    ...
    with st.expander("📌 Alasan dan detail 15M"):
        detail_cols = [c for c in ["Kode", "Alasan", "15M Price", "15M EMA9", "15M EMA20", "15M RSI", "15M Vol x"] if c in bpjs.columns]
        detail = bpjs[detail_cols].copy()
    ...
    st.download_button(
        "⬇️ Download BPJS CSV",
        data=bpjs.to_csv(index=False).encode("utf-8"),
        file_name="bpjs_pagi_sore.csv",
        mime="text/csv",
    )

    c1, c2, c3 = st.columns(3)
    c1.metric("Universe", len(tickers))
    c2.metric("Kandidat Daily", len(daily_bpjs))
    c3.metric("READY 15M", int((bpjs["15M Confirm"] == "READY").sum()))
...
# ------------------------------------------------------------
    ...
    st.caption("15M = arah • 5M = trigger. Fokus indikator intraday, terpisah dari Watchlist Besok dan BPJS.")
    ...
    daily_liquid = scan_bpjs_daily(tickers)
